...
export const comments = pgTable("comments", {
  ...
  userPostIdx: index("idx_comments_user_post").on(table.user_id, table.post_id),
}));
...
export const replies = pgTable("replies", {
  ...
  userCommentIdx: index("idx_replies_user_comment").on(table.user_id, table.comment_id),
}));
...
export const reactions = pgTable("reactions", {
  ...
  targetIdx: index("idx_reactions_target").on(table.target_type, table.target_id),
  ...
  userTargetIdx: index("idx_reactions_user_target").on(table.user_id, table.target_type, table.target_id),
  ...
  targetTypeIdx: index("idx_reactions_target_type").on(table.target_type, table.target_id, table.type),
}));
...
// Additional indexes for the users table
export const usersIndexes = pgTable("users", {
  id: serial("id").primaryKey(),
  name: varchar("name", { length: 255 }),
  email: varchar("email", { length: 255 }).notNull().unique(),
  hashed_password: text("hashed_password").notNull(),
...
export const postsIndexes = pgTable("posts", {
  id: serial("id").primaryKey(),
  title: varchar("title", { length: 255 }).notNull(),
  slug: varchar("slug", { length: 255 }).notNull().unique(),
  content: text("content").notNull(),
  ...
  userCreatedAtIdx: index("idx_posts_user_created_at").on(table.user_id, table.created_at),
}));